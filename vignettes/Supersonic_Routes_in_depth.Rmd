...
[Supersonic Routing](SupersonicRouting.html) provides a quick end-to-end introduction to using the `himach` package and is the place to start. This vignette provides advice on more advanced use, explaining details that the introduction skates over.
...
#not run
hm_load_cache(paste("data/route_start_cache_", p_grid_a@name, 
...
4. If you think you have merged caches from 2 different versions of `himach` in which the contents of a route dataframe have changed, then this is for you. Of course that never happens.


```{r inspecting the cache}
#first create a cache ----
# need to load some of the built-in data
hm_clean_cache() # for this example, start with empty cache
...
hm_save_cache("v0", NZ_grid, aircraft, tempdir()) 

# 
# # Use case 1: How big is my cache? 
# length(.hm_cache$route_cache) # this is number of legs, not routes
# length(.hm_cache$star_cache)
...
# ls(.hm_cache$route_cache, pattern = "NZAA")
# # copy paste from the results into here to remove a single item
# rm("M22_test-NZAA-NZCH--TRUE-TRUE-NA-TRUE-100-12", envir = .hm_cache$route_cache)
# 
# # Use case 3: remove all legs for a given airport
# length(.hm_cache$route_cache)
# rm(list=ls(.hm_cache$route_cache, pattern="NZDN"), envir = .hm_cache$route_cache)
# length(.hm_cache$route_cache)
# 
...
# route_cols <- lapply(1:length(.hm_cache$route_cache), 
#                      function(x) length(get(ls(.hm_cache$route_cache)[1],
#                                             .hm_cache$route_cache)))
