...
length(route_cache) # this is number of legs, not routes
length(star_cache)
...
ls(route_cache, pattern = "NZAA")
# copy paste from the results into here to remove a single item
rm("M22_test-NZAA-NZCH--TRUE-TRUE-NA-TRUE-100-12", envir = route_cache)

# Use case 3: remove all legs for a given airport
length(route_cache)
rm(list=ls(route_cache, pattern="NZDN"), envir = route_cache)
length(route_cache)
...
route_cols <- lapply(1:length(route_cache), function(x) length(get(ls(route_cache)[1], route_cache)))
