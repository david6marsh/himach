...
The version in comments in the chunk above uses the basic `sf` functionality to simplify and buffer. For reasons that we go into [in another vignette](Supersonic_Routing_in_depth.html#bufferaccuracy), it's probably better to use `s2` for adding a buffer. That gives something like the following code (none of which is used further in this vignette).
