...
This vignette provides an end-to-end example of using the `Mach2` package to find the quickest route for a supersonic aircraft that is allowed to fly supersonic over sea, but only subsonic over land.
...
For the vignette, we'll assume that Wellington has a long enough runway. In fact `Mach2` does not currently check whether a runway is long enough for the aircraft selected.
...
Finer resolution is better, because small islands can become larger obstacles. After a no-fly coastal buffer, say around 30km wide, is applied, even a 1km wide island is an obstacle 61km wide. There are some useful starter maps in `rnaturalearth` (use the `countries` versions), but you need to decide what is good enough for your purposes. One test - if you have a 30km buffer do you have aircraft passing between Crete and mainland Greece without slowing down? Then you're missing the island of Antikythera, famous for its ancient, analogue computer.
...
Since the shape file is large, we provide a simplified outline `NZ_coast` and also the buffer `NZ_buffer30` within `Mach2`. These are in the original projection used by Stats NZ. For global work, it's strongly recommended to use one of the following:

* `crs_Atlantic`: "Robinson", a mid 20C classic view, with Atlantic centre
* `crs_Pacific`: A Pacific-centred variant on this with `crs=CRS("+proj=robin +lon_0=180 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs")`.
...
The version in comments in the chunk above uses the basic `sf` functionality to simplify and buffer. For reasons that we go into [in another vignette](AdvancedRouting.html#bufferaccuracy), it's probably better to use `s2` for adding a buffer. That gives something like the following code (none of which is used further in this vignette).
...
# example map, Mach2::map_routes but without any routes
...
target_km=150
system.time(
p_grid <- make_route_grid(NZ_buffer_Pac,"NZ lat-long at 150km",
