...
As with the aircraft, you can load your own data set and then run it through `expand_airports` to add a geo-coded field to it. If you don't have a list, then `expand_airports` will use the dataset from `airportr` package.
...
Finer resolution is better, because small islands can become larger obstacles. After a no-fly coastal buffer, typically around 30km wide, is applied, even a 1km wide island is an obstacle 61km wide. 
...
Since the shape file is large, we provide a simplified outline `NZ_u` and also the buffer `NZ_b`. These are in the original projection used by Stats NZ (in full: `r sf::st_crs(NZ_u)`). It's strongly recommended to use one of the following:
...
# draw a basic map
route_map(NZ_coast, routes, crs = crs_Pacific, fat_map = NZ_buffer)
