...
#' @param \code{show_route}  one of "speed", "aircraft", "time", "circuity" to
...
#'   blue, 0.4)
#' @param \code{refuel_airports} Show the used refuel airports using these
...
map_routes <- function(
  ...
  if (is.data.frame(routes)) {
    if ( !("advantage_h" %in% names(routes))) {
      ...
      routes <- routes %>%
        left_join(rtes %>% select(fullRouteID, advantage_h, circuity),
                  by = "fullRouteID") %>%
        arrange(advantage_h)
    }
    ...
    if (show_route=="circuity"){
      m <- m +  labs(colour = "Circuity\n(best=1.0)") +
        geom_sf(data = routes,
                aes(geometry=st_wrap(prj(gc, crs=crs)), colour=circuity), fill="white",
                size=l_size, lineend="round", alpha=l_alpha) +
        scale_colour_viridis_c()
    }
    if (show_route == "speed"){
