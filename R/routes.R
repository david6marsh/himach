...
distFromLand <- function(long, lat, land) {
  ...
  mp <- st_transform(
    st_cast(
      st_sfc(
        st_multipoint(matrix(c(long, lat), ncol = 2)),
        crs = crs_longlat
      ),
      'POINT'
    ),
...
findGC <- function(subp, withMap_s2, avoidMap_s2, max_depth = 250) {
  if (getOption("himach.verbosity", default = 0) > 2) {
    message("  ", first(subp$phase), " ", first(subp$phaseID), "  ", nrow(subp))
  }
  ...
  if (
    ...
    if (nrow(subp) == 1 | deep_sea) {
      subp %>% select("phase", "phaseID", "id")
    } else {
      subp %>%
        summarise(
    ...
    test_line <- s2::s2_make_line(
      c(first(subp$long), last(subp$long)),
    ...
    if (sea_only) {
      ...
      bind_rows(
        findGC(
          subp %>%
            slice(1:split) %>%
            mutate(phaseID = paste0(.data$phaseID, "0")),
          withMap_s2,
          avoidMap_s2
        ),
        findGC(
          subp %>%
            slice(split:nrow(subp)) %>%
            mutate(phaseID = paste0(.data$phaseID, "1")),
          withMap_s2,
          avoidMap_s2
        )
      )
...
emptyRoute <- function(ac, ap2, fat_map, levels = lattice_phases) {
  data.frame(
    phase = factor(NA, levels = levels),
    phaseID = "0.",
    w_id = 0,
    from = 0,
    to = 0,
    time_h = NA_real_,
    from_long = ap2$from_long,
    from_lat = ap2$from_lat,
    to_long = ap2$to_long,
    to_lat = ap2$to_lat,
    ...
    mutate(
      gc = st_sfc(st_linestring(), crs = crs_longlat), #for want of anything else NULL sfc
      crow = st_gcIntermediate(
        p1 = c(ap2$from_long, ap2$from_lat),
...
find_routes <- function(ac_ids, ap2_ids, aircraft, airports, ...) {
  ...
  if (!is.data.frame(ap2_ids)) {
    ap2_ids <- as.data.frame(ap2_ids, stringsAsFactors = FALSE)
  }
  ...
  routes <- purrr::reduce(
    lapply(1:nrow(combos), function(x) {
      ac <- aircraft[aircraft$id == combos$ac[x], ]
      ap2 <- make_AP2(combos$ap1[x], combos$ap2[x], airports)
      r <- find_route(ac, ap2, ap_loc = airports, ...)
      pb$tick()
...
find_route <- function(
  ...
  if (getOption("himach.verbosity", default = 0) > 0) {
    message("Route:-", ap2$AP2, "----")
  } #route header v refuel subroutes
  ...
  ap2 <- ap2 %>%
    ...
    mutate(
      ADEP = stringr::str_split(.data$AP2, "(<>)|(>)|(<)", simplify = TRUE)[1],
      ADES = stringr::str_split(.data$AP2, "(<>)|(>)|(<)", simplify = TRUE)[2]
    )
  sep <- stringr::str_remove_all(
    ap2$AP2,
  ...
  if (ap2range_ok) {
    ...
    if (getOption("himach.verbosity", default = 0) > 1) {
      message(" Too far for one leg.")
    }
  ...
  if (is.data.frame(cf_subsonic)) {
    if (getOption("himach.verbosity", default = 0) > 1) {
      message(" Adding subsonic, without range bounds.")
    }
    r_subsonic <- find_leg(
      cf_subsonic,
      ap2,
  ...
  if (is.data.frame(refuel) & (!ap2range_ok || !refuel_only_if)) {
    ...
    r_ap3 <- ap2 %>%
      ...
      rowwise() %>%
      mutate(
        dep_ref_km = geosphere::distGeo(
          c(.data$from_long, .data$from_lat),
          c(.data$ref_long, .data$ref_lat)
        ) /
          1000,
        ref_des_km = geosphere::distGeo(
          c(.data$ref_long, .data$ref_lat),
          c(.data$to_long, .data$to_lat)
        ) /
          1000
      ) %>%
      ungroup() %>%
      filter(
        .data$dep_ref_km < ac$range_km - margin_km &
      ...
      mutate(
        circuity = (.data$dep_ref_km + .data$ref_des_km) / ap2$gcdist_km,
    ...
    if (nrow(r_ap3) < 1) {
      ...
      r_ap2 <- r_ap3 %>%
        ...
        mutate(
          ...
          AP2 = ifelse(
            ...
            paste_ADEPADES(.data$ADEP, .data$ADES, FALSE)
          )
        ) #get it in the 'right' order (for the cache)

      # w <- lapply(1:nrow(r_ap2), function(x) find_leg(ac, r_ap2[x, ], fat_map=fat_map, route_grid = route_grid, avoid=avoid,
      #                                                             unidirectional=unidirectional, ...))
      w <- lapply(1:nrow(r_ap2), function(x) {
        find_leg(
          ac,
      ...
      refuel_options <- refuel_options %>%
        mutate(
          legs = ifelse(.data$routeID == ap2$AP2, 1, 2),
          leg_id = ifelse(grepl(ap2$ADEP, .data$routeID), 1, 2),
          refuel_ap = stringr::str_remove_all(
            .data$routeID,
            paste0("(", ap2$ADEP, ")|(<>)|(>)|(", ap2$ADES, ")")
          ),
          routeID = ap2$AP2,
          fullRouteID = paste(ap2$ADEP, .data$refuel_ap, ap2$ADES, sep = sep)
        )

      best_routes <- refuel_options %>%
        group_by(.data$fullRouteID) %>%
        summarise(time_h = sum(time_h), dist_km = sum(.data$gcdist_km)) %>%
      ...
      best_refuel_options <- refuel_options %>%
        ...
        mutate(
          grid_s = utils::head(.data$grid[[1]], 1),
          grid_e = utils::tail(.data$grid[[1]], 1),
          reverse = (.data$leg_id == 1 &
            .data$grid_s !=
              stringr::str_split(.data$routeID, "(<>)|(>)", simplify = TRUE)[
                1
              ]) |
            (.data$leg_id == 2 &
              .data$grid_e !=
      ...
      refuel_legs <- best_refuel_options %>%
        ...
        filter(row_number() == n()) %>%
        mutate(
          phase = factor("refuel", levels = lattice_phases),
          from = .data$to,
          time_h = refuel_h,
          from_long = .data$to_long,
      ...
      sel_routes <- sel_routes %>%
        ...
        #renumber the phases
        mutate(
          phaseChange = case_when(
            row_number() == 1 ~ 1L,
            phase != lag(phase) ~ 1L,
            TRUE ~ 0L
          ),
          phaseID = paste0(
            cumsum(.data$phaseChange),
            ".",
            stringr::str_split(
              .data$phaseID,
              stringr::coll("."),
...
#cached SID-STAR
findToCToD <- function(ap, route_grid, fat_map, ac, ad_dist_m, ad_nearest) {
  ...
  if (
    ...
    if (
      getOption("himach.verbosity", default = 0) > 0 &
        attr(.hm_cache$star_cache, "map") != ""
    ) {
      message("Map or aircraft have changed, so clearing star cache.")
    }
    hm_clean_cache("star")
    attr(.hm_cache$star_cache, "map") <- route_grid@name
    attr(.hm_cache$star_cache, "aircraftSet") <- attr(ac, "aircraftSet")
  ...
  if (!exists(cache_as, envir = .hm_cache$star_cache, inherits = F)) {
    if (getOption("himach.verbosity", default = 0) > 2) {
      message("  TOC/TOD not cached: calculating...")
    }
    assign(
      cache_as,
      findToCToD_really(ap, route_grid, fat_map, ac, ad_dist_m, ad_nearest),
      .hm_cache$star_cache
    )
  }
  #return value
...
findToCToD_really <- function(
  ...
  if (st_crs(fat_map) != use_crs) {
    st_transform(fat_map, use_crs)
  }
  ...
  w <- data.frame(y) %>%
    ...
    group_by(.data$from) %>%
    mutate(
      to = as.character(route_grid@points$id),
      dist_m = units::drop_units(.data$dist_m),
      near_m = abs(.data$dist_m - ad_dist_m)
...
pathToGC <- function(
  ...
  dep <- data.frame(
    phase = factor("arr/dep", levels = levels(route_grid@lattice$phase)),
    phaseID = "0.",
    w_id = 0,
    from = sid[1],
    to = sid[2],
    ...
    left_join(arrDep %>% select(-id, -"arrdep_kph"), by = c("from", "to")) %>%
    #flip the types back to match gcid
    mutate(
      gcdist_km = .data$dist_m / 1000,
      from = 0,
  ...
  arr <- data.frame(
    ...
    #add distance and time for the departure
    left_join(
      arrDep %>% select(-id, -"arrdep_kph"),
      by = c("from" = "to", "to" = "from")
    ) %>%
    #flip the types back to match gcid
    mutate(
      gcdist_km = .data$dist_m / 1000,
      from = as.numeric(.data$from),
      to = 0
    ) %>%
    rename(
      from_long = "long_ap",
  ...
  if (n == 3) {
    ...
    p <- data.frame(
      from = as.numeric(path[1:n - 1]),
      to = as.numeric(path[2:n])
    ) %>%
      left_join(
        route_grid@lattice[, c("from", "to", "phase")],
        by = c("from", "to")
      ) %>%
      left_join(
        route_grid@lattice[, c("from", "to", "phase")],
    ...
    if (ac$over_sea_M == ac$over_land_M | !byTime) {
      ...
      p <- p %>%
        mutate(
          phase = factor("land", levels = levels(route_grid@lattice$phase)),
          phaseID = "1."
        )
    } else {
    ...
    p <- p %>%
      ...
      left_join(route_grid@points[, c("id", "long", "lat")], by = c("id")) %>%
      as.data.frame() %>%
    ...
    if (getOption("himach.verbosity", default = 0) > 2) {
      message("  Ready to recurse")
    }

    gcid <- purrr::reduce(
      lapply(unique(p$phaseID), function(i, m) {
        findGC(p[p$phaseID == i, ], fat_map_s2, avoid_s2)
      }),
      bind_rows
    ) %>%
      #after a single hop line you can start too late, so correct any gaps
      mutate(from = .data$id, to = coalesce(lead(.data$id), last(p$id))) %>%
      rename(w_id = "id") %>%
      filter(!.data$from == .data$to) # not sure why it occasionally churns these out

    if (getOption("himach.verbosity", default = 0) > 1) {
      message(" Done recursion")
    }
    ...
    gcid <- gcid %>%
      ...
      left_join(
        route_grid@points[, c("id", "long", "lat")],
        by = c("from" = "id")
      ) %>%
      ...
      while (baseID < nrow(gcid) - 1) {
        ...
        if (gcid[baseID, ]$phase %in% c("sea", "land")) {
          ...
          while (farID > baseID + 1) {
            ...
            if (nrow(phases) == 1) {
              test_line <- s2::s2_make_line(
                c(gcid[baseID, ]$from_long, gcid[farID, ]$to_long),
              ...
              if (all_sea) {
                if (getOption("himach.verbosity", default = 0) > 2) {
                  message("  Shortcut from ", baseID, " to ", farID)
                }
                #shift the intermediate points onto the new shortcut line
                interm <- (baseID + 1):farID
                old_pts <- s2::s2_lnglat(
                  gcid[interm, ]$from_long,
                  gcid[interm, ]$from_lat
                )
                ...
                gcid[baseID:(farID - 1), c("to", "to_long", "to_lat")] <- gcid[
                  interm,
                ...
                if (
                  farID == nrow(gcid) |
                    gcid[baseID, "phase"] != gcid[farID + 1, "phase"]
                ) {
                  new_baseID <- farID
                } else {
                  ...
                  while (hits_land) {
                    ...
                    test_line <- s2::s2_make_line(
                      c(
                        gcid[new_baseID, ]$from_long,
                        gcid[farID + 1, ]$to_long
                      ),
                ...
                if (new_baseID - baseID > 1) {
                  ...
                  gcid[baseID, c("to", "to_long", "to_lat")] <- gcid[
                    new_baseID,
                    c("from", "from_long", "from_lat")
                  ]
                }
  ...
  gcid <- gcid %>%
    ...
    rowwise() %>%
    mutate(
      gc = st_gcIntermediate(
        p1 = c(.data$from_long, .data$from_lat),
...
#' available rather than re-calculated.
#' See `vignette("Supersonic_Routes_in_depth")` for cache anagement.
#
#'
...
find_leg <- function(
  ...
  if (any(is.na(ac))) {
    stop("Aircraft invalid (check selected row).")
  } #should without NAs
  ...
  if ((attr(.hm_cache$route_cache, "map") != route_grid@name)) {
    ...
    if (
      ...
      message("Map used by grid has changed, so clearing route cache.")
    }
  ...
  if (unidirectional) {
    ...
    cache_as <- paste(
      ac$id,
      min(ap2$ADEP, ap2$ADES),
      max(ap2$ADEP, ap2$ADES),
      attr(avoid, "avoid"),
      enforce_range,
      best_by_time,
      grace_km,
  ...
  if (!exists(cache_as, envir = .hm_cache$route_cache, inherits = F)) {
    ...
    if (is.na(r[1, 1])) {
      (return(r))
    } #quick end without caching if it's an empty route.
...
find_leg_really <- function(
  ...
  if (getOption("himach.verbosity", default = 0) > 0) {
    message("Leg: ", ap2$AP2, " Aircraft: ", ac$type)
  }
  ...
  if (getOption("himach.verbosity", default = 0) > 2) {
    message("  Starting envelope: ", round(Sys.time() - tstart, 1))
  }
  ...
  if (gcdist <= ac$range_km) {
    ...
    if (!is.na(grace_km) & (gcdist / ac$range_km > 0.97)) {
      ac$range_km <- ac$range_km + grace_km
    }
    ...
    if (enforce_range) {
      ...
      if (getOption("himach.verbosity", default = 0) > 1) {
        message(" Cut envelope from lattice: ", round(Sys.time() - tstart, 1))
      }
    }
  }
  ...
  arrDep <-
    bind_rows(
      findToCToD(adep, route_grid, fat_map, ac, ad_dist_m, ad_nearest),
      findToCToD(ades, route_grid, fat_map, ac, ad_dist_m, ad_nearest)
    )
  ...
  if (is.list(avoid)) {
    ...
    if (getOption("himach.verbosity", default = 0) > 2) {
      message("  Adjusted for avoid areas: ", round(Sys.time() - tstart, 1))
    }
  ...
  if (!best_by_time) {
    costed_lattice <- costed_lattice %>%
      mutate(cost = .data$dist_km) %>%
      data.table::as.data.table() #if !bytime, just use distance..
  }
  if (getOption("himach.verbosity", default = 0) > 2) {
    message("  Got costed lattice: ", round(Sys.time() - tstart, 1))
  }

  gr <- makegraph(
    costed_lattice %>% select("from", "to", "cost"),
    directed = FALSE
  )
  ...
  if (length(path[[1]]) > 1) {
    if (getOption("himach.verbosity", default = 0) > 2) {
      message("  Got path: ", round(Sys.time() - tstart, 1))
    }
  ...
  gcArcs <- pathToGC(
    ...
    mutate(
      timestamp = rep(format(Sys.time(), "%Y%m%d_%H%M%OS2"), n()), #unique identifier
      routeID = ap2$AP2,
      acID = ac$id,
      acType = ac$type,
      ...
      envelope = c(
        prj(envelope, crs = crs_longlat),
        rep(st_sfc(st_polygon(), crs = crs_longlat), n() - 1)
      ),
      fullRouteID = ap2$AP2,
      legs = 1,
      leg_id = 1,
  ...
  if ("sea" %in% gcArcs$phase) {
    smoothSpeed(gcArcs, ac)
  } else {
    gcArcs <- gcArcs %>% mutate(speed_kph = .data$gcdist_km / time_h)
  }
}
...
#'
#' @export
make_route_envelope <- function(ac, ap2, envelope_points = 200, fuzz = 0.005) {
  #add a 0.5% fuzz to allow longest range to be flown
  geo_m <- geosphere::distGeo(
    c(ap2$from_long, ap2$from_lat),
    c(ap2$to_long, ap2$to_lat)
  )
  geo_c <- geosphere::midPoint(
    c(ap2$from_long, ap2$from_lat),
    c(ap2$to_long, ap2$to_lat)
  )
...
smooth1 <- function(sign = 1, r, ac) {
  ...
  r_start <- if_else(first(r$speed_kph) < ac$arrdep_kph, 1, 2)
  r_end <- if_else(last(r$speed_kph) < ac$arrdep_kph, nrow(r), nrow(r) - 1L)

  if (sign == 1) {
    seq <- r_start:(nrow(r) - 1L)
  } else {
    seq <- seq.int(r_end, 2, by = -1)
  }

  for (i in seq) {
    r[i, ]$penalty <- ifelse(
      (i - sign) > 0 & (i - sign) <= nrow(r),
      r[i - sign, ]$penalty,
      0
    ) #carry it forward
    if (
      r[i, ]$phase %in%
        c("arr/dep", "transition") &
        r[i + sign, ]$phase == "sea"
    ) {
...
smoothSpeed <- function(r, ac) {
  ...
  r <- r %>%
    mutate(penalty = 0, speed_kph = .data$gcdist_km / .data$time_h) #useful initial value
...
#'
#' @export
summarise_routes <- function(routes, ap_loc, arrdep_h = 0.5) {
  ...
  route_summary <- routes %>%
    ...
    mutate(
      gcdist_km = make_AP2(
        stringr::str_split(first(.data$routeID), stringr::boundary("word"))[[
          1
        ]][1],
        stringr::str_split(first(.data$routeID), stringr::boundary("word"))[[
          1
    ...
    summarise(
      sea_time_frac = round(
        sum(if_else(.data$phase == "sea", .data$time_h, 0)) / sum(.data$time_h),
        3
      ),
      sea_dist_frac = round(
        sum(if_else(.data$phase == "sea", .data$segdist_km, 0)) /
          sum(.data$segdist_km),
        3
      ),
      dist_km = round(sum(.data$segdist_km), 1),
      fly_time_h = round(
        sum(if_else(.data$phase == "refuel", 0, .data$time_h)),
        2
      ),
      time_h = round(sum(.data$time_h), 2),
      n_phases = as.integer(last(.data$phaseID)) -
        as.integer(first(.data$phaseID)) +
        1,
      n_accel = sum(
        if_else(.data$phase == "sea" & lag(.data$phase) != "sea", 1, 0),
        na.rm = TRUE
      )
    ) %>%
    mutate(
      advantage_h = .data$M084_h - .data$time_h,
      advantage_pct = round(.data$advantage_h / .data$M084_h, 3),
      circuity = round(.data$dist_km / .data$gcdist_km, 2) - 1,
      ave_fly_speed_M = round(
        .data$dist_km / (.data$fly_time_h * himach::mach_kph),
        2
      ),
    ) %>%
    ...
    ungroup() %>%
    mutate_at(
      c("dist_km", "n_phases", "n_accel", "circuity"),
